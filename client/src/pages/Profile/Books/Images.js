...
import { EditBook, UploadBookImage } from '../../../api/books';

const Images = ({ selectedBook, setShowBookForm, getData }) => {
    const [file, setFile] = useState(null);
    const [images, setImages] = useState(selectedBook.images);
    ...
    return (
        <div>
                ...
                {images.map((image) => {
                    return (
                        <div className="flex gap-2 border border-solid border-gray-500 rounded p-2 items-end">
                            <img className="h-20 w-20 object-cover" src={image} alt="some-text-here" />
                            <i className="ri-delete-bin-line cursor-pointer text-lg" onClick={() => deleteImage(image)}></i>
                        </div>
                    );
                })}
            </div>
            ...
            >
                <Button type="default" style={{ padding: '8px 16px', height: 'fit-content' }}>
                    Upload Image
                </Button>
            </Upload>

...
import { GetAllBooks, AssignBook, fetchHistory } from '../../api/books';
    ...
    const handleIssue = async () => {
        try {
            ...
            if (response.success) {
                ...
                const history = await fetchHistory(id, user._id, new Date(), new Date(Date.now() + 15 * 24 * 60 * 60 * 1000));
                if (history.success) {
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
                        ...
                        <div className='flex flex-col gap-2 text-lg'>
                            <div className='flex flex-row items-center justify-between'>
                                ...
                                <span
                                    className={`rounded-2xl text-white text-base p-2 ml-2 mr-2 ${book.quantity - book.assignedTo.length > 0 ? 'bg-green-500' : 'bg-red-500'
                                        }`}
                                >
                                    {Number(book.quantity - book.assignedTo.length) > 0 ? 'Available' : 'Not Available'}
                                </span>
                            </div>
                            <Divider />
